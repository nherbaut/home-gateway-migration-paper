...
\IEEEauthorblockA{Orange\\
Paris, France\\
yipingchenfr@gmail.com}
}
...
Among networking elements, the Home Gateway appears to be one with the most diverse functions to handle and thus, with great potential for virtualisation.
...
Although the NFV approach globally pretends bringing operational advantages in terms of CAPEX and OPEX, it is essential to prove them for Home Gateways scenarios where compatibility and versatility are strong requirements. To achieve this goal, we introduce the concept of Surrogate vNF, which makes Home Gateways NVF aware.
The paper highlights a migration path towards full Home Gateway virtualisation and proves its concept through a real implementation and a simulated evaluation on a practical use case related to video streaming.
\end{abstract}

\begin{IEEEkeywords}
Home Gateway, Network Function Virtualisation, OSGI
\end{IEEEkeywords}
...
\section{Migrating to a NVF-based home gateway \label{sec:migrating} using a Surrogate VNF approach}

\input{03_Migrating.tex}
